...
export default class Apis {
    ...
    async getResourceByName(resource, address = this.visionWeb.defaultAddress.hex) {
        switch (resource) {
            case 'TotalEntropyWeight': {
                // resource = 'TotalEntropyWeight';
                resource = 'TotalEntropyWeight';
                break;
            }
            case 'TotalEntropyWeight': {
            ...
            case 'TotalPhotonLimit': {
                resource = 'TotalPhotonLimit';
                break;
            }
            default:
    ...
    async getResourcesByName(resources, address = this.visionWeb.defaultAddress.hex) {
        if (!this.visionWeb.isAddress(address))
            throw new Error('Invalid address provides.')

        const accountResources = await this.visionWeb.vs.getAccountResources(address);
        if (!accountResources)
            throw new Error('Get resource failed.');
    ...
    async getChainParametersByName(keys) {
        const params = await this.visionWeb.vs.getChainParameters();
